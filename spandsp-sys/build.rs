...
fn main() {
    ...
    generate_config_h(&out_dir, fax, v32bis, v34, &env::var("TARGET").unwrap());
...
fn generate_config_h(out_dir: &Path, fax: bool, v32bis: bool, v34: bool, target: &str) {
...
// Only copy the compatibility headers we need: adding vendor/src/msvc directly
// would shadow modern SDK inttypes.h and our generated config.h/spandsp.h.
...
fn compile_generator(out_dir: &Path, vendor_src: &Path, name: &str, sources: &[&str]) -> PathBuf {
    let host = env::var("HOST").unwrap();
    let generator_dir = out_dir.join("generators").join(name);
    fs::create_dir_all(&generator_dir).unwrap();
    generate_config_h(&generator_dir, false, false, false, &host);
    ...
    build
        ...
        .define("_USE_MATH_DEFINES", "1")
        .include(&generator_dir)
        .include(out_dir)
        .include(vendor_src)
        .include(vendor_src.join("../tools"));
    if host.contains("msvc") {
        build.include(prepare_msvc_headers(out_dir, vendor_src));
        // The Godard generator includes getopt.c itself; the filter does not.
        if sources.contains(&"make_modem_filter.c") {
            build.file(vendor_src.join("msvc/getopt.c"));
        }
    }
    for source in sources {
        build.file(vendor_src.join(source));
    }
    let objects = build.compile_intermediates();
    let compiler = build.get_compiler();
    let bin = generator_dir.join(if host.contains("windows") {
        format!("{name}.exe")
    } else {
        name.to_owned()
    });
    let mut command = compiler.to_command();
    command.current_dir(&generator_dir).args(objects);
    if compiler.is_like_msvc() {
        command.arg(format!("/Fe{}", bin.display()));
    } else {
        command.arg("-o").arg(&bin);
        if !host.contains("windows") {
            command.arg("-lm");
        }
    }
    let output = command.output().expect("link native code generator");
    assert!(
...
fn run_generators(out_dir: &Path, vendor_src: &Path, fax: bool, v34: bool) {
    ...
    let compile_and_run =
        |sources: &[&str], extra_args: &[&str], output_file: &str, run_args: &[&str]| {
            ...
            let bin_path = compile_generator(out_dir, vendor_src, &name, sources);
    ...
    let filter_bin = compile_generator(
        out_dir,
        vendor_src,
        "gen_modem_filter",
        &["make_modem_filter.c", "filter_tools.c"],
    );
    ...
    let godard_bin = compile_generator(
        out_dir,
        vendor_src,
        "gen_modem_godard",
        &["make_modem_godard_descriptor.c", "filter_tools.c"],
    );
    ...
    if v34 {
        ...
        let pre_emph_bin = compile_generator(
            out_dir,
            vendor_src,
            "gen_v34_pre_emphasis",
            &[
                "make_v34_tx_pre_emphasis_filters.c",
                "../tools/meteor-engine.c",
            ],
        );
        let status = Command::new(pre_emph_bin)
            .current_dir(out_dir)
            .status()
            .expect("run V.34 pre-emphasis generator");
        assert!(status.success(), "V.34 pre-emphasis generator failed");
...
fn compile_c_sources(
    ...
    if env::var("TARGET").unwrap().contains("msvc") {
        build.include(prepare_msvc_headers(out_dir, vendor_src));
        build.file(vendor_src.join("msvc/gettimeofday.c"));
    }
...
fn run_bindgen(out_dir: &Path, vendor_src: &Path, manifest_dir: &Path, fax: bool) {
    ...
    let mut builder = bindgen::Builder::default()
        ...
        .clang_arg("-DSPANDSP_STATIC")
        .clang_arg("-D_USE_MATH_DEFINES");

    if env::var("TARGET").unwrap().contains("msvc") {
        builder = builder.clang_arg(format!(
            "-I{}",
            prepare_msvc_headers(out_dir, vendor_src).display()
        ));
    }
